'''usage: python -m train_scripts.train_CvM (finds module and runs .py file as script)'''
...
if __name__ == '__main__':
    ...
    identifier = 'learn_clayton_CvM_dim2'
    ...
    optim_args = \
    {
        'lr': 1e-2, # torch.mean for loglikelihood
        'momentum': 0.9
    }
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            with torch.no_grad():
                ...
                c = torch.zeros(zs)
                for j in range(zs):
                    c[j] = torch.mean(torch.prod(data<=data[j,:],axis=1).double())

            cvmloss = torch.sum(torch.pow(net(data)-c.detach(),2))
